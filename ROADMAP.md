...
The existing `repo-ready` typed coordination message means *"my PR merged, your blocker is gone"* — that's a workflow signal, not a boot signal. This phase introduces a separate, explicit *boot-readiness* signal.
...
**Why.** The user's three trust pains all collapse to "I can't see what state the system is in": *Are the right repos wired up? Have agents finished onboarding? Is anything actually working?* Today the answer requires opening the GUI, asking the agent, or grepping `~/.relay/`. The fix is to make the system *announce* its state at every surface the user already touches. This is what makes Relay's multi-repo value proposition feel real instead of theoretical.
